    ...
    for step in history:
        ...
        if winner==0:
          h['value']=0.0
            ...
            if h['to_play'] == winner:
                v=1.0
            elif h['to_play'] == other_player(winner):
                v=-1.0
...
def policy_loss_cross_entropy(predicted_log_probs, target_policy):
    ...
    # pass
    loss=0
    for i in range(len(predicted_log_probs)):
        for j in range(len(predicted_log_probs[i])):
            loss+=predicted_log_probs[i][j]*target_policy[i][j]
...
def combined_loss(predicted_log_probs, predicted_values, target_policy, target_values, net, policy_weight=1.0, value_weight=1.0, l2_weight=1e-4):
    ...
    # pass
    value_mse=value_loss_mse(predicted_values,target_values)
    l2_reg=l2_regularization_loss(net)
    policy_ce=policy_loss_cross_entropy(predicted_log_probs, target_policy)

    combined_loss=value_weight*value_mse + l2_weight*l2_reg + policy_weight*policy_ce
...
def training_epoch(net, optimizer, buffer, batch_size, policy_weight=1.0, value_weight=1.0, l2_weight=1e-4, seed=None):
    ...
    for minibatch in iterate_minibatches(buffer, batch_size, seed):
      step = training_step(net, optimizer, minibatch, policy_weight, value_weight, l2_weight)
      all_total_loss+=step['total']
      all_l2_loss+=step['l2']
      all_policy+=step['policy']
      all_value+=step['value']
      num_batches+=1
    
    all_total_loss=all_total_loss/num_batches
    all_l2_loss=all_l2_loss/num_batches
    all_policy=all_policy/num_batches
    all_value=all_value/num_batches

    ans={'policy':all_policy,'l2':all_l2_loss,'total':all_total_loss,'value':all_value}
...
def self_play_iteration(net, optimizer, num_games, num_simulations, c_puct, batch_size, num_epochs=1, temperature=1.0):
    ...
    for epoch in range(num_epochs):
        if len(buffer) == 0:
            zero_loss = {'total': 0.0, 'policy': 0.0, 'value': 0.0, 'l2': 0.0}
            per_epoch_loss.append(zero_loss)
        else:
            ans = training_epoch(net, optimizer, buffer, batch_size)
            per_epoch_loss.append(ans)
...
def random_policy_action(state, to_play, rng=None):
    ...
    # pass
    valid_cols = valid_moves(state)
    if(rng is None):
        rng = np.random.default_rng()
    
    act = rng.choice(valid_cols)
...
def play_one_match(agent_one, agent_two, starting_player=1):
    ...
    while not done:
        if current_player == 1:
            col = agent_one(board, current_player)
        else:
            col = agent_two(board, current_player)

        board, done, winner, next_player = step_env(board, col, current_player)
...
def match_win_rate(agent_one, agent_two, num_matches, alternate_starts=True):
    ...
    # pass
    player=1
    ans={'wins':0,'losses':0,'draws':0}
    for match in range(num_matches):
        winner = play_one_match(agent_one, agent_two, starting_player=player)
        if(winner==1):
            ans['wins']+=1
        elif(winner==2):
            ans['losses']+=1
